...
\Title{Terrain-Aware Fixed-Line Planning for Multibeam Bathymetric Mapping Using Public Bathymetry Benchmarks}
...
\abstract{Multibeam echo-sounder (MBES) surveys often use inspectable lawnmower plans, but uniform spacing ignores terrain-dependent swath changes. This paper presents a reproducible public-grid benchmark for terrain-aware fixed-line MBES planning from a prior bathymetric grid. The planner combines a depth-referenced footprint approximation, heading search, quantile-based adaptive spacing, and local Genetic Algorithm (GA) refinement while preserving sweep topology. Experiments use two GEBCO~2025 subsets, synthetic tests, and a USGS 30~m multibeam-derived grid. On GEBCO, terrain-aware planning reaches 98.99--99.55 percent predicted coverage, reduces mean excess-overlap violation from about 0.81 to 0.113 percent, and shortens mean path length by 0.75 percent relative to fixed spacing. Ablations show that adaptive spacing explains most public-scene route shortening; GA is a local repeatability/refinement layer. On the USGS high-complexity crop, fixed spacing is infeasible with 29.96 percent excess overlap, whereas the hybrid planner remains feasible under the C97/O3 benchmark gate with 98.42 percent coverage, 1.75 percent excess overlap, and 25 percent path gain. Strict threshold diagnostics expose remaining local-tail risk. The study provides a planning-layer benchmark, not field or hydrographic-quality validation.}
\keyword{Multibeam echo sounder (MBES); Bathymetric surveying; Pre-mission survey-line planning; Public bathymetry benchmark; Coverage path planning; Terrain-aware spacing; Genetic Algorithm (GA)}
...
This study makes five bounded contributions:
\begin{itemize}
    \item It formulates terrain-aware pre-mission CPP for MBES bathymetric survey under a fixed lawnmower traversal, replacing constant spacing with a local sensor--terrain swath model.
    \item It develops an inspectable fixed-line planner in which orientation search and quantile-based adaptive spacing choose the line family, while GA refinement tests small local line-position perturbations and seed-level repeatability without changing the sweep topology.
    \item It provides a reproducible public-bathymetry benchmark that separates external-data evidence from mechanism tests: two GEBCO~2025 public-prior scenes define a reproducible global-grid benchmark, three synthetic terrains expose failure modes, and a USGS 30~m multibeam-derived public grid tests transfer to a higher-resolution source.
    \item It quantifies the actual role of each planning layer. On the GEBCO scenes, adaptive spacing explains nearly all route shortening, while GA contributes only small local refinement and seed-level repeatability; the main mechanism is therefore terrain-aware spacing rather than GA novelty.
    \item It defines validity boundaries using prior-error, coarse-prior/fine-grid replay, execution-uncertainty, margin-selection, current-drift, vehicle-aware, and segmented-heading diagnostics, identifying where fixed-line geometry helps and where platform execution, altitude-aware AUV modeling, or mission logs are still needed.
\end{itemize}
...
\]
The two-side decomposition matters because local relief can make the insonified footprint asymmetric. In the reported benchmark, however, those side contributions are collapsed into a total-width spacing proxy before raster evaluation. A survey-grade extension should preserve explicit port/starboard footprints, roll/pitch/heave effects, and beam-level quality information.
...
Table~\ref{tab:evidence_map} organizes the Results section by evidential role. GEBCO public-prior figures and tables establish reproducibility on globally available gridded bathymetry; USGS and coarse-prior replay cases provide stronger high-resolution transfer evidence; synthetic cases explain the overlap regime in which the method matters; ablation and repeatability separate adaptive spacing from GA cleanup; and robustness replays define what the fixed-line geometry can and cannot support before mission-log or controller-level validation is available. A supplemental four-window GEBCO expansion is reported as a scene-selection risk check rather than as a replacement for the reported GEBCO pair.
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.18\linewidth}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.31\linewidth}>{\raggedright\arraybackslash}p{0.13\linewidth}@{}}
		...
		Ablation and repeatability & Public ablation, 50-seed repeatability, bootstrap CIs, and PSO sanity check. & Adaptive spacing explains most public route shortening; GA adds only local refinement and seed-level repeatability rather than a separate main effect. & No global-optimality or GA-only novelty claim. \\
		...
		Execution boundary & Vehicle-aware post-evaluation, execution-noise replay, uncertainty-aware margins, and current-drift replay. & Moderate perturbations retain useful margin in several public layouts; stronger drift/noise still requires execution-aware planning. & Numerical replay only; no mission logs or sea trials. \\
		\bottomrule[1bp]
	\end{tabular}
\end{table}
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}lrrrrrrrr@{}}
		\toprule[1bp]
		\textbf{Method} & \textbf{Feasible windows} & \textbf{\(G_L\) median (\%)} & \textbf{\(G_L\) mean [95\% CI] (\%)} & \textbf{\(G_L>0\)} & \textbf{\(p_G\)} & \textbf{\(\Delta O\) median (pp)} & \textbf{\(\Delta O\) mean [95\% CI] (pp)} & \textbf{\(\Delta O>0\) / \(p_O\)} \\
		\midrule[1bp]
		Adaptive Spacing & 9/9 & 0.681 & 3.041 [0.198, 8.378] & 7/9 & 0.00977 & 0.799 & 3.895 [0.719, 9.920] & 9/9 / 0.00195 \\
		Hybrid GA & 9/9 & 0.681 & 3.213 [0.315, 8.734] & 7/9 & 0.00977 & 0.663 & 3.869 [0.595, 10.045] & 9/9 / 0.00195 \\
		\bottomrule[1bp]
	\end{tabular}}
\end{table}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.96\textwidth,height=0.34\textheight,keepaspectratio]{pic/journal_metric_heatmap.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\textwidth,height=0.34\textheight,keepaspectratio]{pic/journal_structured_prior_error_replay.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.94\textwidth,height=0.31\textheight,keepaspectratio]{pic/journal_uncertainty_replay.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.94\textwidth,height=0.31\textheight,keepaspectratio]{pic/journal_uncertainty_margin_replay.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\textwidth,height=0.34\textheight,keepaspectratio]{pic/journal_coarse_prior_replay.png}
...
The main text keeps the figures that change the interpretation of the method, while secondary diagnostics are supplied as reproducibility artifacts. The equal-budget PSO comparison, GA surrogate-audit heatmap, full target-overlap matrix, grid-resolution matrix, prior-depth and relief-scale checks, penalty-weight grid, submission-boundary \(W_{\max}\)/GA-gate CSV and JSON files, threshold/local-failure CSV and JSON files, and full current-drift replay heatmap are retained as generated artifacts. Their role is auditability: they allow a reviewer to trace optimizer dependence, surrogate-evaluator agreement, declared-parameter sensitivity, prior-map robustness, local-tail behavior, and execution-boundary claims without turning the Results section into a figure appendix. For the same reason, the current-drift heatmap is not promoted as a visual headline; the manuscript reports the bounded numerical outcome and treats the heatmap as supporting evidence.
...
The synthetic scenes are essential because they expose mechanism and boundary conditions that the public pair alone cannot. In the present dataset, the penalty of fixed spacing rises most sharply once the baseline line family enters a high-overlap regime, which in turn occurs on the more heterogeneous terrains. At the same time, the hardest complex-relief scene remains infeasible even after geometry-aware refinement. This negative result is scientifically important. It suggests that the present design space, which assumes a single global heading and a fixed parallel-line family, becomes too restrictive once relief varies strongly enough within the same region. The transition-aware segmented-heading diagnostic supports that interpretation while avoiding the earlier overclaim risk. On the synthetic stress scene, the selector accepts a four-block plan in all 30 seeds and restores feasibility. On Monterey, it accepts blockwise plans in 14 of 30 seeds and otherwise retains the single-heading plan, which indicates that segmentation can improve the mission-time proxy after the coverage gate. On Mariana and Puerto Rico, it rejects every blockwise candidate and falls back to the single-heading layout because the apparent route savings are obtained by losing coverage. On the USGS high-complexity crop, it rejects the blockwise layouts that the coverage-preserving selector would have accepted, because the added transition burden is not justified by the overlap cleanup. The repair should therefore not be over-interpreted as a complete mission planner, because it still lacks full Dubins route construction, closed-loop vehicle control, current, field execution, and mission-log validation. Its value is narrower but useful: it identifies a gated transition from a single-heading benchmark toward operation-relevant fixed-pattern survey design.
