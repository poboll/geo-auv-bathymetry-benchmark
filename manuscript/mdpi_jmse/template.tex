...
\Title{Terrain-Aware Fixed-Line MBES Survey Planning with Gate-Aware Blockwise Repair Using Public Bathymetric Priors}
...
\abstract{Multibeam echo-sounder (MBES) surveys often rely on inspectable parallel-line plans, yet uniform spacing ignores terrain-driven swath variation. This study addresses pre-mission fixed-line survey planning with an available prior bathymetric grid. We present a reproducible public-grid benchmark and a terrain-aware planning framework that combines a depth-referenced footprint approximation, heading search, quantile-based adaptive spacing, lightweight GA local refinement, and a gate-aware blockwise fixed-line extension for single-heading failures. Tests use two GEBCO~2025 public-prior windows, three synthetic mechanism tests, and a USGS Southern Cascadia 30~m multibeam-derived grid. On the GEBCO pair, terrain-aware layouts retain 98.99--99.55 percent predicted coverage, reduce mean excess-overlap violation from about 0.81 to 0.113 percent, and shorten mean path length by 0.75 percent relative to fixed spacing. Across nine public windows, overlap cleanup is positive in 9/9 cases (Wilcoxon \(p=0.00195\); rank-biserial 1.00), while route gain is regime dependent. The USGS high-complexity crop shows the strongest stressed result: Fixed-Spacing is infeasible at 29.96 percent excess overlap, whereas the hybrid layout remains feasible with about 25 percent path gain. Blockwise repair restores feasibility on the synthetic complex-terrain failure but is accepted only when coverage, overlap, and transition-cost gates justify it.}
\keyword{Multibeam echo sounder (MBES); Survey-line design; Fixed-line planning; Public bathymetric prior; Reproducible benchmark; Terrain-aware spacing; Blockwise fixed-line planning; Robustness diagnostics}
...
\begin{itemize}
    ...
    \item It adds a gate-aware blockwise fixed-line extension that keeps each block inspectable, but accepts segmented headings only when coverage, overlap, and transition-cost gates justify the added complexity.
    \item It separates mechanism from local cleanup: public-window and stress-regime analyses show that adaptive spacing is the primary source of overlap discipline, whereas GA is an optional gated refinement layer and the threshold, prior-error, uncertainty, current-drift, vehicle-aware, footprint-validity, external-baseline, and blockwise-decision audits define the validity boundary.
\end{itemize}
...
\paragraph{Gate-aware blockwise fixed-line extension}
The single-heading formulation is deliberately inspectable, but it is also the most restrictive part of the method. For complex-relief stress cases, we therefore evaluate a constrained blockwise extension rather than an unconstrained free-form path planner. The survey rectangle is divided into a small number of cross-track blocks, each block keeps a fixed parallel-line family, and each block receives its own terrain-aware heading and adaptive-spacing profile. Candidate block counts \(K\in\{2,3,4,5\}\) are generated from the same sensor--terrain evaluator as the single-heading benchmark. The reported path length includes within-block survey lines, conservative inter-block boundary transits, and a heading-change term evaluated with a declared minimum turning radius of \(R_{\min}=100\)~m.

The extension is gate-aware. A segmented candidate must satisfy the same \(C97/O3\) coverage/overlap screen as the main benchmark and must not reduce coverage relative to the single-heading Hybrid GA unless it converts an infeasible single-heading layout into a feasible one. We then report two selectors. The coverage-preserving selector chooses the lower \(L+3O_{\mathrm{ex}}\) score among the single-heading and accepted blockwise candidates. The transition-aware selector ranks the same survivors by a mission-time proxy at \(R_{\min}=100\)~m, with survey and transit distance evaluated at 1.5~m/s and turn arcs at 0.75~m/s. This blockwise variant is treated as a formal stress-case extension to the fixed-line framework: it tests whether limited heading segmentation repairs the single-heading failure mode while preserving line-family auditability. It is not a Dubins controller, online route optimizer, or vehicle-dynamics model.
...
The failure visualization also motivates the gate-aware blockwise extension defined in Methods: the planner needs more than one fixed-line heading when relief changes substantially across the same survey box. The extension splits the survey rectangle into a small number of cross-track blocks, keeps a fixed line family inside each block, and scores candidate combinations with boundary-transit and heading-change terms. A segmented candidate is first accepted only if it satisfies the global 97 percent coverage and 3 percent excess-overlap gates and does not reduce coverage relative to the single-heading Hybrid GA unless it converts an infeasible single-heading layout into a feasible one. After this gate, the coverage-preserving selector ranks accepted candidates by \(L+3O_{\mathrm{ex}}\), whereas the transition-aware selector uses the declared \(R_{\min}=100\)~m mission-time proxy with line-change arcs and slower turn speed. This is a formal stress-case extension to the fixed-line framework, not a hidden free-form route planner.
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=\textwidth,height=0.30\textheight,keepaspectratio]{pic/journal_segmented_decision_audit.png}
	\caption{Decision audit for the gate-aware blockwise fixed-line extension, recomputed from \texttt{segmented\_heading\_extension/segmented\_heading\_raw.csv}. Panel (a) reports how many of the 30 seeds select a blockwise layout under the coverage-preserving selector and the stricter transition-aware selector. Panel (b) reports the trade-off relative to the single-heading Hybrid GA: coverage-preserving overlap cleanup in percentage points and transition-aware mission-time change in percent. The audit shows three distinct outcomes: mandatory repair on synthetic Complex Terrain, selective acceptance on Monterey Canyon, and rejection when coverage loss or transition burden outweighs the blockwise benefit.}
	\label{fig:segmented_decision_audit}
\end{figure}
...
The main text keeps the figures that change the interpretation of the method, while secondary diagnostics are supplied as reproducibility artifacts. The equal-budget PSO comparison, external heuristic layout audit, external turning-cost audit, segmented decision audit, GA surrogate-audit heatmap, full target-overlap matrix, grid-resolution matrix, prior-depth and relief-scale checks, penalty-weight grid, submission-boundary \(W_{\max}\)/GA-gate CSV and JSON files, threshold/local-failure CSV and JSON files, side-specific footprint validity CSV/JSON/PNG files, and full current-drift replay heatmap are retained as generated artifacts. Their role is auditability: they allow a reviewer to trace optimizer dependence, external baseline dependence, turn-radius cost dependence, segmented-heading accept/reject decisions, surrogate-evaluator agreement, declared-parameter sensitivity, prior-map robustness, evaluator simplification, local-tail behavior, and execution-boundary claims without turning the Results section into a figure appendix. For the same reason, the current-drift heatmap is not promoted as a visual headline; the manuscript reports the bounded numerical outcome and treats the heatmap as supporting evidence.
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}p{0.24\linewidth}p{0.32\linewidth}p{0.34\linewidth}@{}}
		...
		Blockwise fixed-line decision audit & \texttt{make\_segmented\_decision\_audit.py} & \texttt{segmented\_decision\_audit/} CSV, JSON, README, and PNG outputs \\
...
The synthetic scenes are essential because they expose mechanism and boundary conditions that the public pair alone cannot. In the present dataset, the penalty of fixed spacing rises most sharply once the baseline line family enters a high-overlap regime, which in turn occurs on the more heterogeneous terrains. At the same time, the hardest complex-relief scene remains infeasible even after geometry-aware refinement. This negative result is scientifically important. It suggests that the present design space, which assumes a single global heading and a fixed parallel-line family, becomes too restrictive once relief varies strongly enough within the same region. The gate-aware blockwise extension is therefore a constrained second variant inside the fixed-line family rather than a separate mission planner. Its selection logic is part of the contribution: it repairs the synthetic stress case in 30/30 seeds, accepts Monterey blockwise layouts only when they improve the declared trade-off, rejects Mariana and Puerto Rico candidates whose route shortening comes from coverage loss, and retains the USGS High single-heading solution when transition cost outweighs extra overlap cleanup. This keeps the paper focused on fixed-line geometry while making the single-heading boundary actionable instead of merely descriptive.
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\linewidth}>{\raggedright\arraybackslash}p{0.44\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}@{}}
		...
		Complex-terrain failure & The infeasible single-heading case is retained as a boundary result; the gate-aware blockwise fixed-line extension repairs the synthetic stress case and rejects blockwise layouts when transition cost or coverage loss is not justified. & Blockwise segmentation is a numerical fixed-line extension, not a full vehicle or mission planner. \\
