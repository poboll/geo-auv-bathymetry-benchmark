...
def make_figure(summary_rows: list[dict[str, float | str | int]]) -> None:
    ...
    fig = plt.figure(figsize=(7.35, 3.62), facecolor=jhs.BG)
    grid = fig.add_gridspec(2, 2, wspace=0.040, hspace=0.150)
    ...
    fig.subplots_adjust(left=0.112, right=0.998, top=0.934, bottom=0.052, wspace=0.040, hspace=0.150)
    jhs.save_white_rgb(fig, OUT / "uncertainty_replay.png", pad_inches=0.010)
    for pic_dir in PIC_DIRS:
        jhs.save_white_rgb(fig, pic_dir / "journal_uncertainty_replay.png", pad_inches=0.010)
    plt.close(fig)
